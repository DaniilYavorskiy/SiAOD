...
void ShakerSort (int A[], int n) {
	...
	k=n-1;
	do {
		for(j=right; j>left; j-- ){ // от правого к левому
			Cprak++;
			if (A[j]<A[j-1]) { // обмен
				w=A[j];
		  	 	A[j]=A[j-1];
		...
		left=k;
		for(j=left; j<right; j++ ){ // от левого к правому
			Cprak++;
	   		if(A[j]>A[j+1]){ // обмен
	   			w=A[j];
	  	 		A[j]=A[j+1];
...
void InsertSort (int A[], int n) {
	...
	for(i=1; i<n; i++) {
		...
		t=A[i];
		j=i-1;
		while(j>=0 && (Cprak++, t<A[j])) { // пока елемент меньше, смешаем
			Mprak++;
			A[j+1]=A[j];
			j=j-1;
		}
...
void SelectSort (int A[], int n) 
{
	Mprak = 0, Cprak = 0;
	int i,j,k,t;
	for (i = 0; i < n-1; i++ ) { // начальное значение min
		k = i;
		for( j=i+1; j<n; j++){ // находим index min
			Cprak++;
	   		if (A[j]<A[k])k=j;
		}
		t=A[i];
		A[i]=A[k];
		A[k]=t; // меняем значения
		Mprak+=3;
	}
}
