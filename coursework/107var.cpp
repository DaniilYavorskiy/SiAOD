...
int compare(const Record &record1, const Record &record2) {
    int birth_date_comparate_arr[] = {0, 1};
    for (int i : birth_date_comparate_arr) {
...
int quick_search(Record *arr[], const std::string &key) {
    ...
    while (l < r) {
        int m = (l + r) / 2;
        if (strcomp(&arr[m]->birth_date[0], key, 2) < 0) {
            l = m + 1;
        } else {
            r = m;
        }
    }
    if (strcomp(&arr[r]->birth_date[0], key, 2) == 0) {
        return r;
    }
    return -1;
}
...
void SDPREC(Record *D, Vertex *&p) {
    if (!p) {
        ...
        p->left = nullptr;
        p->right = nullptr;
    } else if (D->department < p->data->department) {
        SDPREC(D, p->left);
    } else if (D->department >= p->data->department) {
        SDPREC(D, p->right);
    }
...
void search(Record *arr[], int &ind, int &n) {
    ...
    if (ind == -1) {
        ...
        tail = head;
        int i;
        for (i = ind + 1; strcomp(&arr[i]->birth_date[0], key, 2) == 0; ++i) {
